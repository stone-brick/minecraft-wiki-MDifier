...
class TestCache:
    ...
    def test_save_then_load(self, cache_dir):
        """保存后能加载回"""
        from minecraft_wiki_mdifier.cache import load_cache, save_cache
    ...
    def test_expired_entries_filtered(self, cache_dir):
        """过期条目（> 7 天）被过滤"""
        from minecraft_wiki_mdifier.cache import CACHE_TTL, load_cache
    ...
    def test_clear_cache_existing(self, cache_dir):
        """clear_cache 删除存在的文件"""
        from minecraft_wiki_mdifier.cache import clear_cache
    ...
    def test_cache_info_existing(self, cache_dir):
        """cache_info 9 字段"""
        from minecraft_wiki_mdifier.cache import cache_info, save_cache
